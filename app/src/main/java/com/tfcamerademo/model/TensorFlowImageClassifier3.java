...
public class TensorFlowImageClassifier3 implements Classifier {
    ...
    public static Classifier create(Context context, ImageView imageview, AssetManager assetManager, String modelFilename, String labelFilename,
        ...
        try {
            classifier.detector = Detector.create(assetManager, modelFilename);
        } catch (Exception e) {
            Log.e(TAG, "Error creating detector", e);
        }
    ...
    public List<Recognition> recognizeImage(final Bitmap bitmap) {
        ...
        try {
            Detector.Detections detections = detector.detect(bitmap);
            for (Detector.Detection detection : detections) {
                // Process the detection result
                for (Detector.Label label : detection.getLabels()) {
                    // Process the label
                }
            }
        } catch (Exception e) {
